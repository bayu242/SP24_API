...
* **Teacher:** Menggunakan @@map("teachers") untuk penamaan tabel jamak di MySQL namun tetap tunggal di kode program. Ditambah kolom refresh_token untuk sesi login.  
...
* **POST /api/auth/refresh-token**  
  * **Fungsi:** Memperbarui akses token yang kedaluwarsa.  
  * **Body:** { "refreshToken": "..." }

### **2. Modul Teacher (Manajemen Guru)**

* **GET /api/teachers** - Mengambil semua daftar guru (Wajib Admin).
* **POST /api/teachers** - Menambahkan guru baru (Wajib Admin).
* **GET /api/teachers/:id** - Mengambil detail profil 1 guru (Wajib Admin).  
* **PUT /api/teachers/:id** - Mengedit data guru (Wajib Admin).  
* **DELETE /api/teachers/:id** - Menghapus data guru dari sistem (Wajib Admin).
* **POST /api/teachers/:id/photo** - Mengunggah foto profil guru (Multipart/form-data).
* **GET /api/teachers/:id/photo** - Mengambil file foto profil guru secara langsung (Public).

### **3. Modul Student (Manajemen Siswa & Kartu NFC)**

* **GET /api/students** - Menampilkan semua data siswa (Akses: Admin & Teacher).  
* **GET /api/students/:id** - Mengambil detail profil 1 siswa (Akses: Admin & Teacher).  
...
* **POST /api/students/:id/photo** - Mengunggah foto profil siswa (Multipart/form-data).
* **GET /api/students/:id/photo** - Mengambil file foto profil siswa secara langsung (Public).

### **4. Modul Presence (Absensi)**

* **GET /api/presences**  
  * **Fungsi:** Mengambil semua rekapitulasi riwayat data absensi.  
  * **Auth:** Wajib Token (Admin/Guru).  
* **POST /api/presences/tap**  
  * **Fungsi:** Mencatat absensi (Masuk/Pulang) via Tap NFC.  
  * **Body:** { "tag_id": "...", "teacher_id": 1 }  
* **GET /api/presences/parent/:nis**  
  * **Fungsi:** Portal orang tua untuk melihat riwayat absensi anaknya (Tanpa Token).

### **5. Modul WebSocket (Jembatan Pendaftaran Kartu)**

* **Event Klien ke Server:** request-pairing-code, android-send-tag, check-pairing-code
* **Event Server ke Klien:** pairing-code-generated, nfc-received, pairing-error, device-connected, pairing-verified
